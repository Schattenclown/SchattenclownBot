...
namespace SchattenclownBot.Integrations.Discord.Services
{
    ...
    {
        public async Task GenerateBirthdayList()
        {
            new CustomLogger().Information("Starting GenerateBirthdayList...", ConsoleColor.Green);
            ...
            foreach (KeyValuePair<ulong, DiscordGuild> guildItem in guildsList)
            {
                if (guildItem.Value.Id == 928930967140331590)
                {
                    DiscordChannel chn = await DiscordBot.DiscordClient.GetChannelAsync(928938948221366334);
        ...
        public void RunAsync(int executeSecond)
        {
            ...
            Task.Run(async () =>
            {
                ...
                while (true)
                {
                    ...
                    {
                        DiscordGuild discordGuildObj = DiscordBot.DiscordClient.GetGuildAsync(discordGuild.Id).Result;
                        IReadOnlyDictionary<ulong, DiscordMember> discordMembers = discordGuildObj.Members;
                        foreach (KeyValuePair<ulong, DiscordMember> discordMemberItem in discordMembers)
                        {
                            DateTime birthday = GenerateDateTime(discordMemberItem.Value);
        ...
        public DateTime GenerateDateTime(DiscordMember discordMember)
        {
            string birthdayDay = "";
            int birthdayMonth = 0;
            ...
            DiscordGuild discordGuildObj = DiscordBot.DiscordClient.GetGuildAsync(discordMember.Guild.Id).Result;
