...
internal class VoteSystem : ApplicationCommandsModule
{
	...
	public static async Task GiveRatingAsync(InteractionContext interactionContext, [Option("User", "@...")] DiscordUser discordUser)
	{
		...
		DiscordStringSelectComponentOptionList[3] = new DiscordStringSelectComponentOption("Rate 4", "rating_4", emoji: new DiscordComponentEmoji("💎"));
		DiscordStringSelectComponentOptionList[4] = new DiscordStringSelectComponentOption("Rate 5", "rating_5", emoji: new DiscordComponentEmoji("👑"));

		DiscordStringSelectComponent DiscordStringSelectComponent = new("Select a Rating!", DiscordStringSelectComponentOptionList, "give_rating");

		await interactionContext.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral().AddComponents(DiscordStringSelectComponent).WithContent($"Give <@{discordUser.Id}> a Rating!"));
	}
	...
	public static async Task GiveRatingAsync(ContextMenuContext contextMenuContext)
	{
		...
		await contextMenuContext.CreateResponseAsync(InteractionResponseType.ChannelMessageWithSource, new DiscordInteractionResponseBuilder().AsEphemeral().AddComponents(DiscordStringSelectComponent).WithContent($"Give <@{contextMenuContext.TargetMember.Id}> a Rating!"));
	}
